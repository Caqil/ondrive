...
type SendLocationMessageRequest struct {
	...
	IsLiveLocation bool    `json:"is_live_location,omitempty"`
	LiveDuration   int     `json:"live_duration,omitempty"` // minutes
}
...
type UpdateChatSettingsRequest struct {
	...
	BlockUnknownSenders  *bool   `json:"block_unknown_senders,omitempty"`
}
...
// FileUploadService interface (should match your existing implementation)
type FileUploadService interface {
	UploadChatImage(file interface{}, path string) (*UploadResult, error)
	UploadChatAudio(file interface{}, path string) (*UploadResult, error)
}
...
func (cc *ChatController) DeleteConversation(c *gin.Context) {
	...
	if conversation.CreatedBy.Hex() != userID {
		utils.ForbiddenResponse(c)
		return
	}
...
func (cc *ChatController) UpdateMessage(c *gin.Context) {
	...
	// Can only update text messages
	if message.Type != models.MessageTypeText {
		utils.BadRequestResponse(c, "Only text messages can be updated")
		return
	}
...
func (cc *ChatController) SendLocationMessage(c *gin.Context) {
	...
	// Set location data
	locationData := models.MessageLocation{
	...
	if req.IsLiveLocation && req.LiveDuration > 0 {
		...
		locationData.LiveUntil = &liveUntil
	}
...
func (cc *ChatController) GetRideChat(c *gin.Context) {
	...
	messages, err := cc.messageRepo.GetLatestMessages(conversation.ID.Hex(), 50)
	if err != nil {
...
func (cc *ChatController) StartEmergencyChat(c *gin.Context) {
	...
	if err != nil {
		cc.logger.Warn().Err(err).Msg("Failed to create initial emergency message")
	}
...
func (cc *ChatController) GetEmergencyChatContacts(c *gin.Context) {
	...
	utils.SuccessResponse(c, http.StatusOK, "Emergency chat contacts retrieved successfully", emergencyContacts)
}
...
// Helper response functions (add these to your utils package if not exists)
func CreatedResponse(c *gin.Context, message string, data interface{}) {
	utils.SuccessResponse(c, http.StatusCreated, message, data)
}
